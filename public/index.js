...
document.addEventListener('DOMContentLoaded', () => {
  search.addEventListener('click', async () => {
    ...
    try {
      ...
      // Event listener for the Astronomy Data button
      const astronomyDataButton = document.querySelector('.astronomy-data-button');
      astronomyDataButton.addEventListener('click', async () => {
        clearUI();
        try {
          const response = await fetch('/astronomy-data');
          const astronomyData = await response.json();

          // Update the UI with astronomy data
          const astronomyDataElement = document.querySelector('.astronomy-data');
          astronomyDataElement.innerHTML = generateAstronomyDataHTML(astronomyData);
        } catch (error) {
          console.error('Fetch Astronomy Data Error:', error);
        }
      });
      ...
      function generateAstronomyDataHTML(astronomyData) {
        let html = `<p>Sunrise: ${astronomyData.sunrise}</p>`;
        html += `<p>Sunset: ${astronomyData.sunset}</p>`;
        html += `<p>Moonrise: ${astronomyData.moonrise}</p>`;
        html += `<p>Moonset: ${astronomyData.moonset}</p>`;
        return html;
      }
      ...
      function clearUI() {
        ...
        // Clear astronomy data-related elements
        const astronomyDataElement = document.querySelector('.astronomy-data');
        astronomyDataElement.innerHTML = '';
